...
void pushfunc(SymbolTableEntry_t * sym){
		...
		newNode->symbol = sym;
		newNode->jump = nextInstructionLabel() - 1;
		if (funcStack == NULL){
			newNode->next = NULL;
		}else{
...
void generate_FUNCSTART (quad* quad_){
		...
		tempInstr->arg2 = NULL;
		tempInstr->result->type = label_a;
		tempInstr->result->val = nextInstructionLabel();
		emit_instr(tempInstr);
...
void generate_RET(quad* quad_){
	...
	make_retvaloperand(tempInstr->result);
	if(quad_->result != NULL){
		make_operand(quad_->result, tempInstr->arg1);
	}else{
		tempInstr->arg1 = NULL;
	}
	tempInstr->arg2 = NULL;
...
void generate_FUNCEND(quad* quad_){
		...
		instractions[f->jump].result->val = nextInstructionLabel()+1;
...
bool_ iterate_symbol_binary(void *data, int i){
  ...
  unsigned int address = temp->taddress;
...
void printINBytes(void){
	...
	unsigned int prVarsCounter = programVarOffset;
	...
	/* NUm of global var*/	
	unsigned int * gv = malloc (sizeof (unsigned int));
	gv = &prVarsCounter;
	printf("GLOBAL VARS: %d\n" , programVarOffset);
	fwrite(gv,sizeof(unsigned int),1,f);
	...
	for (i = 0; i<= currInstr; i++){
		...
		unsigned int * line = malloc (sizeof (unsigned int));
		line = &(instractions[i].srcLine);
		fwrite(line,sizeof(unsigned int),1,f);	
